...
async def search_degree(page, url, document, degree_name):
    ...
    try:
        ...
        logger.info(f"Navigating to TJSP {degree_name} for document: {document} (cleaned: {clean_document})")
        ...
        try:
            ...
            logger.warning(f"Timeout waiting for networkidle in {degree_name}. Saved error_screenshot_{degree_name}.png")
            pass

        # Check for "No results" message
        content = await page.content()
        if "Não existem processos" in content or \
           "Nenhum processo foi encontrado" in content or \
           "Não existem informações disponíveis para os parâmetros informados" in content:
        ...
        process_items = await page.query_selector_all("div.processoDetalhes, tr.fundoClaro, tr.fundoEscuro")
        ...
        if not process_items:
            ...
            for item in process_items:
                ...
                for line in lines:
                    line = line.strip()
                    prefixes = [
                        "Reqte:", "Reqdo:", "Autor:", "Réu:", "Exectdo:", "Exequente:", 
                        "Agravante:", "Agravado:", "Averiguado:", "Indiciado:", 
                        "Requerente:", "Requerido:", "Impetrante:", "Impetrado:",
                        "Interessado:", "Embargante:", "Embargado:", "Apelante:", "Apelado:"
                    ]
        ...
        # Fallback: If process_items were found but no details were extracted (e.g. selector mismatch)
        if not details and count > 0:
            logger.warning(f"Process items found ({count}) but no details extracted. Trying direct link extraction.")
            process_links = await page.query_selector_all("a.linkProcesso")
            if process_links:
...
async def search_portal_transparencia(document: str):
    ...
    async with async_playwright() as p:
        ...
        try:
            # Clean document (keep only numbers)
            clean_document = "".join(filter(str.isdigit, document))
            logger.info(f"Navigating to Portal da Transparência for document: {document} (cleaned: {clean_document})")
            
            # Correct URL based on subagent findings
            await page.goto("https://portaldatransparencia.gov.br/busca/", timeout=60000, wait_until="domcontentloaded")
            ...
            try:
                ...
                await page.wait_for_selector("ul#resultados div.busca-portal-block-searchs__item", timeout=20000)
            except:
            ...
            link_element = await page.query_selector("ul#resultados div.busca-portal-block-searchs__item a")
            ...
            if href:
                ...
                # If href starts with /busca/ and we are at /busca/, it might be fine if we use root domain
...
async def search_tjsp(document: str):
    ...
    async with async_playwright() as p:
        ...
        import asyncio
        task1 = search_degree(page1, "https://esaj.tjsp.jus.br/cpopg/open.do", document, "1º Grau")
        task2 = search_degree(page2, "https://esaj.tjsp.jus.br/cposg/open.do", document, "2º Grau")
